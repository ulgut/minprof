    ...
    E: FnMut(&[u8], &mut Vec<T>) -> usize + Send + 'static,
{
    ...
    let extractor_thread = thread::Builder::new()
        ...
        .spawn(move || {
            let mut extract_fn = extractor; // move into mutable local so we can call FnMut
            ...
            // Dropping send_batch signals EOF to the main thread.
        })?;
...
fn process_impl(path: &Path, visitor: &mut dyn RecordVisitor) -> Result<()> {
    // -- File header ----------------------------------------------------------
    ...
    let parser = thread::Builder::new()
        ...
        .spawn(move || {
            let mut parser = HprofRecordParser::new(id_size);
