...
use passes::edges::{Pass2Output, EDGE_SIZE};
use passes::index::{Pass1Output, ENTRY_SIZE, load_class_index, load_roots};
...
fn write_meta(
    ...
    for v in [1u64, object_count, root_count, edge_count,
...
fn index_is_complete(dir: &Path) -> bool {
    ["object_index.bin", "class_names.bin", "retained.bin", "meta.bin",
     "edges.bin", "reverse_edges.bin"]
        .iter()
...
fn load_index(dir: &Path) -> Result<(Pass1Output, Pass2Output, Pass4Output)> {
    ...
    let object_count = std::fs::metadata(&object_index_path)
        .context("stat object_index.bin")?.len() / ENTRY_SIZE as u64;
    ...
    let edge_count   = std::fs::metadata(&edges_path)
        .context("stat edges.bin")?.len() / EDGE_SIZE as u64;
...
fn run_query(
    ...
    match cli.format {
        Format::Html => {
            ...
            let html_path = cli.profile.as_deref()
